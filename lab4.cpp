...
int main()
{
	Mat data1 = imread("D:\\摄影测量\\摄影测量实验四\\data\\10040084.tif");
	int imw1 = data1.cols;
	int imh1 = data1.rows;
	...
	FILE* fp_in= fopen("D:\\摄影测量\\摄影测量实验四\\data\\left2.txt", "r");
	int num = 38;//
	...
	for (int i=0;i<num;i++)
	{
		fscanf(fp_in, "%lf%lf", &ix1[i], &iy1[i]);
		cout << "左影像读入成功！" << endl;
	}
	
	fclose(fp_in);
	fp_in = NULL;
	Mat data2 = imread("D:\\摄影测量\\摄影测量实验四\\data\\10040083.tif");
	int imw2 = data2.cols;
	int imh2 = data2.rows;

	FILE* fp_in2 = fopen("D:\\摄影测量\\摄影测量实验四\\data\\right2.txt", "r");
	...
	for (int i = 0; i < num; i++)
	{
		ix1[i] = (ix1[i] - imw1 / 2)*0.012;
		iy1[i] = (iy1[i] - imh1 / 2)*0.012;
		ix2[i] = (ix2[i] - imw2 / 2)*0.012;
		iy2[i] = (iy2[i] - imh2 / 2)*0.012;
	}
	...
	float S1a1, S1a2, S1a3, S1b1, S1b2, S1b3, S1c1, S1c2, S1c3, S2a1, S2a2, S2a3, S2b1, S2b2, S2b3, S2c1, S2c2, S2c3;
	...
	FILE* fp_out = fopen("D:\\摄影测量\\摄影测量实验四\\data\\res.txt", "w");
		for (int i = 0;i < num;i++)
		{
